  ...
  try {
    ...
    await pool.query(`
      CREATE TABLE IF NOT EXISTS uic_job_candidates (
        ...
        herramientas_mecanica TEXT DEFAULT '[]',
        instrumentos_electrica TEXT DEFAULT '[]'
      )
    `);
...
app.post("/jobs/candidates", async (req, res) => {
  try {
    ...
    const herramientas_mecanica = Array.isArray(b.herramientas_mecanica) ? b.herramientas_mecanica.slice(0, 80).map((x)=>_sanitizeText(x,120)).filter(Boolean) : [];
    const instrumentos_electrica = Array.isArray(b.instrumentos_electrica) ? b.instrumentos_electrica.slice(0, 80).map((x)=>_sanitizeText(x,120)).filter(Boolean) : [];
    ...
    await pool.query(
      `INSERT INTO uic_job_candidates (
        ...
        herramientas_mecanica, instrumentos_electrica,
      ...
      [
        ...
        ultimo_trabajo || "",
        observaciones || "",
        JSON.stringify(herramientas_mecanica || []),
...
// Export XLSX (Excel real) – SOLO ADMIN
app.get("/jobs/export", requireAdmin, async (req, res) => {
  try {
    ...
    const aoa = [headers].concat(
      items.map((it) => headers.map((h) => esc(it[h])))
    );
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="uic_bolsa_trabajo_${new Date().toISOString().slice(0, 10)}.xlsx"`
    );
    return res.send(buf);
  } catch (e) {
